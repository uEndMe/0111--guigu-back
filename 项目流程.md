...
axiosInstance.interceptors.response.use(
    ...
    (e)=>{
    	...
    	if(err.response){
            errMessage = 
                errCode[err.response.status] || '响应失败';
        }else{
            //没有响应
...
axiosInstance.interceptors.response.use(
    ...
    (err) => {
    	...
    	if(err.response){
            ...
            if(err.message.indexOf('Netword Error') !== -1){
                errMsg = '网络故障';
            }else if (err.message.indexOf('timeout') !== -1){
                errMsg = '连接超时'
            }
  ...
  return dispatch => {
	...
	return reqLogin(username, password)
	  ...
	  .then((response)=>{
		...
		setItem('user',reponse);
        //触发更新（redux）
        dispatch(saveUser(reponse));
        //跳转页面
		this.props.history.replace('/');
	  })
	  //登录失败
...
import { SAVE_USER } from '../action-types';
import { getItem } from '../utils/sotrage';
